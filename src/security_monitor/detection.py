"""People and new-object detection (Windows + Linux, OpenCV-based)."""
...
# VOC / MobileNet-SSD class id for person.
_PERSON_CLASS_ID = 15
...
def models_dir() -> Path:
    xdg = Path.home() / ".cache" / "security-monitor" / "models"
...
class PersonDetector:
    """Prefer MobileNet-SSD (downloaded once); fall back to HOG on failure."""
    ...
    @property
    def ready(self) -> bool:
        return self._backend in {"dnn", "hog"}
    ...
    def _ensure_backend(self) -> None:
        with self._lock:
            ...
            try:
                ...
                weights = models_dir() / _MOBILENET_MODEL
                if not proto.is_file():
                    _download(_MOBILENET_PROTOTXT_URL, proto)
                if not weights.is_file():
                    _download(_MOBILENET_MODEL_URL, weights)
                net = cv2.dnn.readNetFromCaffe(str(proto), str(weights))
                self._net = net
                self._backend = "dnn"
                return
            except Exception as exc:  # noqa: BLE001 - fall back to HOG
    ...
    def detect(self, frame: np.ndarray, *, conf_thresh: float = 0.45) -> list[Box]:
        self._ensure_backend()
        if frame is None or frame.size == 0:
            return []
        h, w = frame.shape[:2]
        if self._backend == "dnn" and self._net is not None:
            return self._detect_dnn(frame, w, h, conf_thresh)
        if self._backend == "hog" and self._hog is not None:
            return self._detect_hog(frame, w, h)
        return []

    def _detect_dnn(self, frame: np.ndarray, w: int, h: int, conf_thresh: float) -> list[Box]:
...
class NewObjectTracker:
    """Compare against a saved baseline to find packages / left-behind objects."""
    ...
    def detect(
        ...
        *,
        ...
        # Ignore person regions so walking people are not "new objects".
        if person_boxes:
        ...
        min_area = max(400, int(w * h * min_area_ratio))
        candidates: list[Box] = []
        for contour in contours:
            x, y, bw, bh = cv2.boundingRect(contour)
            if bw * bh < min_area:
                continue
            # Ignore huge full-frame changes (lighting / camera move).
            if bw * bh > 0.45 * w * h:
                continue
        ...
        with self._lock:
            ...
            for cand in candidates:
                ...
                if matched is not None:
                    ...
                    if not matched.confirmed and now - matched.first_seen >= confirm_seconds:
                        matched.confirmed = True
                    updated.append(matched)
                    tracks.remove(matched)
                else:
                    updated.append(
                        _TrackedObject(box=cand, first_seen=now, last_seen=now, confirmed=False)
                    )
            # Keep briefly-missing confirmed objects so boxes don't flicker out.
            for track in tracks:
                if track.confirmed and now - track.last_seen < 1.0:
...
class DetectionEngine:
    ...
    def __init__(self) -> None:
        ...
        self._interval = 0.35  # seconds between heavy detects per camera
        self._lock = threading.Lock()
    ...
    def process(
        ...
        *,
        ...
        if detect_people:
            people_boxes = self.people.detect(work)
            if scale != 1.0:
                people_boxes = [
                    Box(
                        int(b.x1 / scale),
        ...
        if detect_objects:
            # Run object diff in original resolution for stable baseline match.
            # Exclude people using full-res boxes.
        ...
        boxes = [*people_boxes, *object_boxes]
        with self._lock:
